...
| `ranking_metric`     | null   | Structure prediction metric to use for ranking designs (e.g., `'af2_pae_interaction'`, `'boltz_ptm_interface'`, `'boltz_ipSAE_min'`). Must match prediction method prefix (`af2_*` or `boltz_*`). If null, defaults to `'af2_pae_interaction'` for AlphaFold2 or `'boltz_ipSAE_min'` for Boltz-2. See [Metrics Guide](metrics.md) for all available metrics. |
